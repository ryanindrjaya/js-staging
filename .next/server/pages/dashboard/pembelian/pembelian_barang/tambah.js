...
function SearchBar({
  ...
  const fetchProduct = async (query, callback) => {
    if (!query) {
      ...
      try {
        const endpoint = "https://js-strapi.keelola.net/api" + `/products?populate=*&filters[$or][0][name][$contains]=${query}&filters[$or][1][SKU][$contains]=${query}`;
        const options = {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
        ...
        if (req.status == 200) {
          ...
          const filteredProduct = res.data.filter(item => {
            return !(selectedProduct !== null && selectedProduct !== void 0 && selectedProduct.some(temp => temp.id == item.id));
          }); // product based on user location

          const filteredProductByLocation = filteredProduct.filter(item => item.attributes.locations.data.some(location => user.locations.some(userLocation => userLocation.id == location.id)));
