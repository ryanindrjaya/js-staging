    ...
    if (!query) {
      ...
      try {
        const endpoint =
          ...
          `/sales-sells?&filters[status][$eq]=Sebagian Diterima&filters[status][$eq]=Diproses&filters[customer][id][$eq]=${customerId}&filters[no_sales_sell][$contains]=${query}&sort[0]=id:desc`;
        const options = {
          method: "GET",
          headers: {
