...
export default function SalesPaymentTable({ data }) {
  ...
  const getProductDisc = (row) => {
    ...
    for (let index = 1; index < 6; index++) {
      var unitProduct = row.attributes?.product.data.attributes[`unit_${index}`];
  ...
  const columns = [
    ...
    },
    {
      ...
      width: "150px",
      selector: (row) => {
        return `${row.attributes?.qty} ${row.attributes?.unit}`;
      },
    },
