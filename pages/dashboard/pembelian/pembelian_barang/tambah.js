...
function Tambah({ props }) {
  ...
  async function fetchLatestNoReferensi() {
    ...
    const endpoint = `${process.env.NEXT_PUBLIC_URL}/purchasings?sort[0]=id:desc&pagination[limit]=1&filters[no_purchasing][$contains]=${codename}/PB/`;
    const headers = {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
    ...
    if (response) {
      ...
      const no = parseInt(latestDaata?.attributes?.no_purchasing?.split("/")?.[2] || 0) + 1;
      console.log("no", no);
      const latestNoReferensi = `${codename}/PB/${String(no).padStart(5, "0")}/${moment().format("MM/YYYY")}`;
      form.setFieldsValue({
        no_purchasing: latestNoReferensi,
      });
