...
const fetchPiutang = async (cookies, id) => {
  const endpoint = process.env.NEXT_PUBLIC_URL + "/credit-details?populate=*&filters[panel_sale][id][$eq]=" + id;
  const options = {
    method: "GET",
    headers: {
      "Content-Type": "application/json",
...
const fetchPaymentRetur = async (cookies, id) => {
  const endpoint = process.env.NEXT_PUBLIC_URL + "/retur-panel-sales?populate=*&filters[panel_sale][id][$eq]=" + id;
  const options = {
    method: "GET",
    headers: {
      "Content-Type": "application/json",
...
function ReturPanel({ props }) {
  ...
  async function fetchLatestNoReferensi() {
    ...
    const endpoint = `${process.env.NEXT_PUBLIC_URL}/retur-panel-sales?sort[0]=id:desc&pagination[limit]=1&filters[no_retur_panel_sale][$contains]=${codename}/RP/`;
    const headers = {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
    ...
    if (response) {
      ...
      const no = parseInt(latestDaata?.attributes?.no_retur_panel_sale?.split("/")?.[2] || 0) + 1;
      console.log("no", no);
      const latestNoReferensi = `${codename}/RP/${String(no).padStart(5, "0")}/${moment().format("MM/YYYY")}`;
      form.setFieldsValue({
        no_retur_panel_sale: latestNoReferensi,
      });
