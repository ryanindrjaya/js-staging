...
const Tambah = ({ props }) => {
  ...
  const [kodeAkun, setKodeAkun] = useState(`AP/${user.id}/${noAkun}/${mm}/${yyyy}`);
  //Akun COA
  ...
  const onFinish = async (values) => {
    ...
    if (req.status === 200) {
      ...
      router.replace("/dashboard/penjualan/panel/setting");
    } else {
...
const fetchAkun = async (cookies) => {
    const endpoint = process.env.NEXT_PUBLIC_URL + "/store-accounts?populate=*&filters[penjualan]=PANEL";
    const options = {
        method: "GET",
        headers: {
            "Content-Type": "application/json",
