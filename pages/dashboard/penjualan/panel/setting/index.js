...
const fetchAkun = async (cookies) => {
    const endpoint = process.env.NEXT_PUBLIC_URL + "/store-accounts?sort[0]=setting%3Adesc&sort[0]=type%3Aasc&filters[penjualan]=PANEL&populate=chart_of_account";
    const options = {
        method: "GET",
        headers: {
            "Content-Type": "application/json",
...
function Setting({ props }) {
    ...
    const handleUpdate = (id) => {
       router.push("/dashboard/penjualan/panel/setting/edittambahakun" + id);
    };
    ...
    const handleDelete = async (id) => {
        ...
        if (res) {
            ...
            openNotificationWithIcon(
                ...
                "Akun panel yang dipilih telah berhasil dihapus. Silahkan cek kembali penjualan panel"
            );
            setAkun(res);
        }
    };
    ...
    const handleTambahAkun = () => {
        router.push("/dashboard/penjualan/panel/setting/tambahakun");
    };
    ...
    const handleChangeSetting = async (values, id) => {
        ...
        if (req.status === 200) {
            ...
            openNotificationWithIcon("success", "Setting berhasil dirubah", "Setting berhasil dirubah. Silahkan cek setting panel");
        } else {
    ...
    return (
        <>
            <Head>
                <title>AKUN PANEL</title>
            </Head>
            <DashboardLayout>
                <LayoutWrapper style={{}}>
                    <TitlePage titleText={"AKUN PANEL"} />
                    <LayoutContent>
