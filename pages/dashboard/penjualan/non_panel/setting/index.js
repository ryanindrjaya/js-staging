...
const fetchAkun = async (cookies) => {
    const endpoint = process.env.NEXT_PUBLIC_URL + "/store-accounts?sort[0]=setting%3Adesc&sort[0]=type%3Aasc&filters[penjualan]=NON&populate=chart_of_account";
    const options = {
        method: "GET",
        headers: {
            "Content-Type": "application/json",
...
function Setting({ props }) {
    ...
    const handleUpdate = (id) => {
       router.push("/dashboard/penjualan/non_panel/setting/edittambahakun" + id);
    };
    ...
    const handleDelete = async (id) => {
        ...
        if (res) {
            ...
            openNotificationWithIcon(
                ...
                "Akun non panel yang dipilih telah berhasil dihapus. Silahkan cek kembali penjualan non panel"
            );
            setAkun(res);
        }
    };
    ...
    const handlePageChange = async (page) => {
        const cookies = nookies.get(null, "token");
        const endpoint = process.env.NEXT_PUBLIC_URL + "/store-accounts?pagination[page]=" + page;
    ...
    const handleTambahAkun = () => {
        router.push("/dashboard/penjualan/non_panel/setting/tambahakun");
    };
    ...
    const onChangeSetting = (setting, row) => {
        ...
        if (aktifLength < 1 || setting == "Tidak Aktif") {
            ...
            router.push("/dashboard/penjualan/non_panel/setting");
        }
    };
    ...
    const handleChangeSetting = async (values, id) => {
        ...
        if (req.status === 200) {
            ...
            openNotificationWithIcon("success", "Setting berhasil dirubah", "Setting berhasil dirubah. Silahkan cek setting non panel");
        } else {
    ...
    return (
        <>
            <Head>
                <title>AKUN NON PANEL</title>
            </Head>
            <DashboardLayout>
                <LayoutWrapper style={{}}>
                    <TitlePage titleText={"AKUN NON PANEL"} />
                    <LayoutContent>
