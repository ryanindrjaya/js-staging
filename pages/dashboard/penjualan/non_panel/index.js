...
function NonPanelSale({ props }) {
  ...
  const print = () => {
    router.replace(
      {
        pathname: "/dashboard/penjualan/non_panel/print/" + selectedData.id,
      },
      undefined,
      { shallow: true }
    );
  ...
  const handleDownloadExcel = async () => {
    ...
    await writeExcel({
      api: "non-panel-sale",
      schema: {
        "No. Penjualan": "no_non_panel_sale",
      ...
      outputPath: "Export Penjualan Non Panel.xlsx",
    });
              ...
              {selectedData && (
                <>
                  <Descriptions
                    ...
                    size="middle"
                    title="INFORMASI PENJUALAN NON PANEL"
                    bordered
                  >
                    <Descriptions.Item label="Tanggal Penjualan" span={4}>
                      {selectedData?.attributes?.sale_date}
                    </Descriptions.Item>
                    <Descriptions.Item label="No Penjualan Non Panel" span={4}>
                      {selectedData?.attributes?.no_non_panel_sale}
                    </Descriptions.Item>
