...
const Edit = ({ data }) => {
  ...
  return (
    <>
      ...
      <DashboardLayout>
        <LayoutWrapper style={{}}>
          <TitlePage titleText={"Edit Supplier"} />
          <LayoutContent>
            <Form form={form} name="add_supplier" initialValues={attributes} onFinish={onFinish}>
                    ...
                    <Input.Group compact>
                      <Form.Item noStyle className="w-full relative" name="credit_limit_duration">
                        <InputNumber
                          ...
                          defaultValue={attributes?.credit_limit_duration || 0}
                          style={{ width: "30%" }}
                        />
...
Edit.getInitialProps = async (ctx) => {
  ...
  const res = await fetch(`${process.env.NEXT_PUBLIC_URL}/suppliers/${id}`, {
    headers: {
